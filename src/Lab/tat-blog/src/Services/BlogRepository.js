...
export async function getPosts(keyword = '', pageSize = 10, pageNumber = 1, sortColumn = '', sortOrder = '') {
    try {
        ...
        const response = await axios.get(`https://localhost:44309/api/posts?keyword=${keyword}&PublishedOnly=true&UnPublished=false&PageSize=${pageSize}&PageNumber=${pageNumber}&SortColumn=${sortColumn}&SortOrder=${sortOrder}`);
